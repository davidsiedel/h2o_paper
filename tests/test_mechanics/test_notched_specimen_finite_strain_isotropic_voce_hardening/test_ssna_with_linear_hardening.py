...
class TestMecha(TestCase):
    def test_ssna_with_linear_hardening(self):
        # --- VALUES
        u_min = 0.0
        u_max = 8.e-4
        time_steps: list = list(np.linspace(u_min, u_max, 51, endpoint=True))
        iterations = 10
        ...
        mesh_file_path = "meshes_comp/ssna.msh"
        # mesh_file_path = "meshes_comp/unit_square_cpp_500.msh"
        ...
        # Mesh(mesh_file_path=mesh_file_path, integration_order=finite_element.computation_integration_order)
        # print("--- %s seconds for mesh reading ---" % (time.time() - start_time))
        #
        # return
        ...
        p = Problem(
            ...
            tolerance=1.0e-6,
            res_folder_path=get_current_res_folder_path() + "_SSNA_WITH_LINEAR_HARDENING_HIGH_STAB_0_DOT_001"
        )
