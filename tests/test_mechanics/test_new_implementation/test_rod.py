...
class TestMecha(TestCase):
    def test_rod(self):
        ...
        u_max = 0.013
        u_s = 0.010088
        u_s_eps = 0.010088 * 0.01
        # time_steps_0 = np.linspace(u_min, u_s - u_s_eps, 20, endpoint=False)
        # time_steps_1 = np.linspace(u_s - u_s_eps, u_s, 20, endpoint=False)
        # time_steps_2 = np.linspace(u_s, u_s + u_s_eps, 20, endpoint=False)
        # time_steps_3 = np.linspace(u_s + u_s_eps, u_max, 20, endpoint=True)
        time_steps: list = list(np.linspace(u_min, u_max, 50, endpoint=True))
        # time_steps: list = list(time_steps_0) + list(time_steps_1) + list(time_steps_2) + list(time_steps_3)
        ...
        damage_boundary_conditions = [
            BoundaryCondition("LEFT", fixed, BoundaryType.PRESSURE, 0),
            BoundaryCondition("RIGHT", fixed, BoundaryType.PRESSURE, 0),
            #
            # BoundaryCondition("LEFT", fixed, BoundaryType.DISPLACEMENT, 0),
        ...
        displacement_mat = Material(
            ...
            # library_path="bhv_displacement_phasefield_deviatoric_damage/src/libBehaviour.so",
            # library_path="bhv_displacement/src/libBehaviour.so",
            # library_name="PhaseFieldDisplacementDeviatoricSplit",
            library_path="bhv_displacement_phasefield_deviatoric_damage/src/libBehaviour.so",
            library_name="PhaseFieldDisplacementDeviatoricSplit",
        ...
        res_folder_path = get_current_res_folder_path() + "_ROD_100_FIXED_PT2_SHEAR_STRESS_NO_ZERO_DAMAGE4"
        ...
        cp.make_time_step_fixed_point(
            ...
            external_variable_type_u=ExternalVariable.FIELD_VARIABLE,
        )
