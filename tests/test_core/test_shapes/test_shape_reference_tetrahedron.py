...
class TestReferenceTetrahedron(TestCase):
    ...
    def test_reference_tetrahedron_quadrature(self, verbose=True):
        """
        ...
        for _io in range(1, 9):
            ...
            val = scheme.integrate(f, [v0, v1, v2, v3],)
            # --- GET H20 ESTIMATION
            val_num = 0.0
            ...
            val_scp = integrate.tplquad(f_scipy, 0.0, 1.0, lambda x: 0.0, lambda x: 1.0 - x, lambda x, y: 0.0, lambda x, y: 1.0 - x - y)
            if verbose:
    ...
    def test_reference_triangle_cell(self, verbose=True):
        ...
        for polynomial_order in polynomial_orders:
            for element_type in element_types:
                ...
                scheme = quadpy.t3.get_good_scheme(finite_element.construction_integration_order)
