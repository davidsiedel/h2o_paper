...
class TestCellQuadrangle(TestCase):
    def test_polygon_geometry_cell(self, verbose=True):
        ...
        if verbose:
            ...
            print("-- volume :\n{}".format(v_c))
            for i in range(6):
                plt.scatter(polygon_vertices[0, i], polygon_vertices[1, i], c="b")
            plt.scatter(x_c[0], x_c[1], c="b")
            plt.show()

    def test_polygon_quadrature_cell(self, verbose=True):
        """
        ...
        for _io in range(1, 9):
            ...
            val = scheme.integrate(f, [[v0, v1], [v3, v2]],)
            # --- GET H20 ESTIMATION
            val_num = 0.0
            quadrature_points = shape.get_quadrature_points(_io)
            quadrature_weights = shape.get_quadrature_weights(_io)
            quadrature_size = shape.get_quadrature_size(_io)
            for _qc in range(quadrature_size):
                x_qc = quadrature_points[:, _qc]
                w_qc = quadrature_weights[_qc]
                val_num += w_qc * f(x_qc)
            # --- GET SCIPY ESTIMATION
            val_scp = integrate.dblquad(f_scipy, 0.0, 1.0, lambda x: 0.0, lambda x: 1.0)
            if verbose:
                ...
                for i in range(6):
                    plt.scatter(polygon_vertices[0, i], polygon_vertices[1, i], c="b")
                for _qc in range(quadrature_size):
                    _x_qc = quadrature_points[:, _qc]
                    plt.scatter(_x_qc[0], _x_qc[1], c="g")
                plt.show()

    def test_cell_polygon(self, verbose=True):
        """
        ...
        for polynomial_order in polynomial_orders:
            for element_type in element_types:
                ...
                # --- BUILD CELL
                cell_triangle = Shape(ShapeType.POLYGON, polygon_vertices)
                    ...
                    for basis_1, coef_1 in zip(bases, coefs):
                        ...
                        for _i in range(euclidean_dimension):
                            for _j in range(euclidean_dimension):
                                ...
                                mass_integral_check = scheme.integrate(f_mass_check, [[v0, v1], [v3, v2]])
                                stif_integral_check = scheme.integrate(f_stif_check, [[v0, v1], [v3, v2]])
                                advc_integral_check = scheme.integrate(f_advc_check, [[v0, v1], [v3, v2]])
                                rtol = 1.0e-12
                                atol = 1.0e-12
                                if verbose:
                                    print(
