...
def solve_newton_local_equilibrium2(problem: Problem, material: Material, verbose: bool = False, debug_mode: DebugMode = DebugMode.NONE):
    ...
    with open(output_file_path, "a") as outfile:
        ...
        while time_step_index < len(problem.time_steps):
            ...
            while iteration < problem.number_of_iterations and not break_iteration:
                ...
                local_max_num_iter: int = 10
                local_tolerance: float = 1.0e-6
                for _element_index, element in enumerate(problem.elements):
                    # local_time_step_index: int = 0
                    # local_time_steps: List[ndarray] = [faces_unknown_vector.copy()]
                    # local_time_step_temp: ndarray = local_time_steps[0]
                    # while local_time_step_index < len(local_time_steps):
                    #     local_faces_unknown_vector = local_time_steps[local_time_step_index]
                    #     print(np.max(np.abs(local_faces_unknown_vector)))
                    #     local_iteration: int = 0
                    #     break_local_iteration: bool = False
                    while local_iteration < local_max_num_iter and not break_local_iteration and not break_iteration:
                        ...
                        if not break_local_iteration:
                            ...
                            element_internal_forces += (
                                    ...
                                    * element.stabilization_operator
                                    @ element.get_element_unknown_vector(local_faces_unknown_vector)
                            )
                            # SOLVING LOCAL SYSTEM
                            ...
                            local_external_forces_coefficient = 1. * material.stabilization_parameter / np.prod(bdc)
                            # local_external_forces_coefficient = 1. / np.prod(bdc)
                            ...
                            if local_residual_evaluation < local_tolerance:
                                ...
                                break_local_iteration = True
                            elif local_iteration == local_max_num_iter - 1:
                                # --- SPLITTING TIME STEP
                                # local_time_steps.insert(local_time_step_index, np.copy(
                                # faces_unknown_vector_previous_step) + local_faces_unknown_vector / 2.)
                                # print(
                                #     "++++++++++++ @ CELL : {} | MAX ITERATIONS WITH RES : {:.6E}".format(
                                #         _element_index, local_residual_evaluation))
                                # print("++++++++++++ @ CELL : {} | SPLITTING LOCAL CORRECTION".format(_element_index))
                                # --- DECONDENS
                            ...
                            # print("++++++++++++ @ CELL : {} | SPLITTING LOCAL CORRECTION".format(_element_index))
                            # print("++++++++++++ @ CELL : {} | DECOND".format(_element_index))
                            # face_correction = np.zeros((_nf * _fk * _dx), dtype=real)
                            # for _i_local, _i_global in enumerate(element.faces_indices):
                            #     _c0_fg = _i_global * (_fk * _dx)
                            #     _c1_fg = (_i_global + 1) * (_fk * _dx)
                    ...
                    if _condtest > 1.e10:
                        ...
                        outfile.write(
                            "++++++++++++++++ @ CELL : {} | CONDITIONING : {}".format(_element_index, _condtest))
                        outfile.write("\n")
