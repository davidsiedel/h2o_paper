            ...
            while iteration < problem.number_of_iterations and not break_iteration:
                ...
                for _element_index, element in enumerate(problem.elements):
                    ...
                    for _qc in range(cell_quadrature_size):
                        ...
                        transformation_gradient = element.get_field_gradient(faces_unknown_vector, _qc)
                        material.mat_data.s1.gradients[_qp] = transformation_gradient
                        # --- INTEGRATE BEHAVIOUR LAW
                        integ_res = mgis_bv.integrate(material.mat_data, material.integration_type, _dt, _qp, (_qp + 1))
                        material.mat_data.K[_qp] = np.eye(transformation_gradient.shape[0])
                        material.mat_data.s1.thermodynamic_forces[_qp] = material.mat_data.K[_qp] @ material.mat_data.s1.gradients[_qp]
                        print(material.mat_data.s1.gradients[_qp])
                        # print(material.mat_data.K[_qp])
                        # print(material.mat_data.K[_qp])
                                ...
                                for f_local, f_global in enumerate(element.faces_indices):
                                    if f_global in problem.mesh.faces_boundaries_connectivity[
                                        ...
                                        for _qf in range(face_quadrature_size):
                                            ...
                                            force_item += (
                                                # _w_q_f * v @ face_lagrange / bdf_proj[0]
                                                    (_w_q_f * material.lagrange_parameter) * v @  face_lagrange / face.get_diameter()
                                                    # (_w_q_f * material.lagrange_parameter) * v @  face_lagrange / face.get_diameter()
                                                    # (material.lagrange_parameter) * v @  face_lagrange
                                                    # _w_q_f * v[0] * face_lagrange[0]
                                                # v @ face_lagrange
                                                # face_lagrange[0]
                                            )
                                        ...
                                        # force_item = material.lagrange_parameter * (v[0] * face_lagrange[0])
                                        # force_item = material.lagrange_parameter * (v[:] @ face_lagrange[:])
