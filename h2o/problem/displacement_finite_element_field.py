...
class DisplacementFiniteElementField:
    ...
    loads: List[Load]
    tolerance: float
    elements: List[DisplacementElement]
    res_folder_path: str
    max_iterations: int
    ...
    def get_element_internal_forces(self, element_index: int) -> (bool, ndarray, ndarray):
        ...
        element: DisplacementElement = self.elements[element_index]
    ...
    def make_condensation(self, element_index: int, element_residual: ndarray, element_stiffness_matrix: ndarray):
        ...
        # print("condens element : {} | pos : {} | eig max {} | stab {}".format(element_index, element.cell.get_centroid(), np.max(np.linalg.eig(m_cell_cell)[0]), self.material.stabilization_parameter))
